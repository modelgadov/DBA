...
SET @BODY=@BODY+N'<br></br>'+'Conexión a Servidor de Aplicaciones:'

declare @gmhsrvm4app table(
resultado varchar(1000))
insert 	@gmhsrvm4app (resultado) 
EXEC xp_cmdshell 'ping gmhsrvm4app.gym.com.pe'
...
set @BODY=@BODY+		
                   ...
                   N'<html><head><style>      
					...
					N'<H3 style="font-size:11pt;color:black;font-family:''Calibri''">   </H3>' +    
					N'<table cellpadding=0 cellspacing=0 border=0>' +        
					N'<tr bgcolor=#F7FE2E > <th>ping gmhsrvm4app.gym.com.pe</th>' +    
					N'</tr>'+       
					CAST ((select td= resultado,''
					from @gmhsrvm4app where resultado is not null     
					FOR XML PATH('tr'), TYPE         
					) AS NVARCHAR(MAX) ) +        
					N'</table></html>'
...
Update @table_disk set Size= 60 where drive ='C'
Update @table_disk set Size= 509 where drive ='D'
Update @table_disk set Size= 269 where drive ='E'
Update @table_disk set Size= 100 where drive ='F'
Update @table_disk set Size= 100 where drive ='G'
Update @table_disk set Size= 70 where drive ='P'
