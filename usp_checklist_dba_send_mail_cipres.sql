...
declare @cipres table(
resultado varchar(1000))
insert 	@cipres (resultado) 
EXEC xp_cmdshell 'ping cipres.gym.com.pe'
...
set @BODY=@BODY+		
                   ...
                   N'<html><head><style>      
					...
					N'<H3 style="font-size:11pt;color:black;font-family:''Calibri''">   </H3>' +    
					N'<table cellpadding=0 cellspacing=0 border=0>' +        
					N'<tr bgcolor=#F7FE2E > <th>ping cipres.gym.com.pe</th>' +    
					N'</tr>'+       
					CAST ((select td= resultado,''
					from @cipres where resultado is not null     
					FOR XML PATH('tr'), TYPE         
					) AS NVARCHAR(MAX) ) +        
					N'</table></html>'														
...
Update @table_disk set Size= 249 where drive ='D'
Update @table_disk set Size= 165 where drive ='E'
