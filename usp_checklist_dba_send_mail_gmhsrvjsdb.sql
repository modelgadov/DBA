...
/****** Object:  StoredProcedure [dbo].[usp_checklist_dba_send_mail]    Script Date: 10/11/2016 09:57:47 ******/
SET ANSI_NULLS ON
GO
...
SET @BODY='GMH - Checklist SQL Server de la instancia '+@@SERVERNAME+' fecha: '+@Fecha+':'+N'<br></br>'
...
SET @BODY=@BODY+N'<br></br>'+'Conexión a Sevidor Web:'

declare @GMHSRVJSWEB table(
resultado varchar(1000))
insert 	@GMHSRVJSWEB (resultado) 
EXEC xp_cmdshell 'ping GMHSRVJSWEB.gym.com.pe'
...
set @BODY=@BODY+		
                   ...
                   N'<html><head><style>      
					...
					N'<H3 style="font-size:11pt;color:black;font-family:''Calibri''">   </H3>' +    
					N'<table cellpadding=0 cellspacing=0 border=0>' +        
					N'<tr bgcolor=#F7FE2E > <th>ping GMHSRVJSWEB.gym.com.pe</th>' +    
					N'</tr>'+       
					CAST ((select td= resultado,''
					from @GMHSRVJSWEB where resultado is not null     
					FOR XML PATH('tr'), TYPE         
					) AS NVARCHAR(MAX) ) +        
					N'</table></html>'						
...
Update @table_disk set Size= 50 where drive ='C'
Update @table_disk set Size= 30 where drive ='D'
Update @table_disk set Size= 104 where drive ='E'
Update @table_disk set Size= 50 where drive ='F'
Update @table_disk set Size= 50 where drive ='G'
Update @table_disk set Size= 50 where drive ='H'
Update @table_disk set Size= 50 where drive ='P'
...
DECLARE @subject_temp VARCHAR(100)      
		  SET @subject_temp='GMH - Checklist SQL Server - '+@@SERVERNAME+'-'+  @Fecha 
